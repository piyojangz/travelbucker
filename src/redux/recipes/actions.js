...
export function getFavourites(dispatch) {
  ...
  return ref.on('value', (snapshot) => {
    ...
    return dispatch({type: 'FAVOURITES_REPLACE', data: favs});
  });
}
...
export function resetFavourites(dispatch) {
  return dispatch({type: 'FAVOURITES_REPLACE', data: []});
}
...
export function getRecipes() {
  ...
  return dispatch => new Firebase.Promise((resolve) => {
    ...
    return ref.on('value', (snapshot) => {
      ...
      return resolve(dispatch({type: 'RECIPES_REPLACE', data: recipes}));
    });
  });
}
...
/**
  * Get OutcomeData
  */
export function getOutcomeData() {
  outcomedata = "dmcfkwempk3mpck3empkewmpp";
  return resolve(dispatch({type: 'GETOUTCOMEDATA', data: {
      outcomedata
    }}));
}